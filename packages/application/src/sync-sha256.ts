...
export function sha256HexUtf8(input: string): string {
  ...
  for (let offset = 0; offset < message.length; offset += 64) {
    ...
    for (let index = 16; index < 64; index += 1) {
      const previous15 = word(schedule, index - 15);
      const previous2 = word(schedule, index - 2);
      ...
      const sigma1 = rotateRight(previous2, 17)
        ...
        ^ (previous2 >>> 10);
      schedule[index] = (
        word(schedule, index - 16)
        + sigma0
        + word(schedule, index - 7)
        + sigma1
      ) >>> 0;
    }
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 = (h + sum1 + choose + word(ROUND_CONSTANTS, index)
        + word(schedule, index)) >>> 0;
