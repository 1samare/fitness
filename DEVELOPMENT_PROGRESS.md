...
| 3. 营养计算与审核数据 | 第 5–6 周 | 未开始 | `feat/v1.0` 尚未实现宏量营养计算、审核营养快照和食谱模板 |
...
- 下一阶段：阶段 3“营养计算与审核数据”。
- 开发入口：先确认科学来源与策略常量，再为营养边界和无解场景编写失败测试，最后实现确定性计算与审核数据接口。
- 开始阶段三时，先把总览和阶段三状态改为“进行中”；完成后同步本文件中的验收证据、剩余工作、外部阻塞和下一阶段。
...
- [ ] 实现蛋白质分支、脂肪与饱和脂肪、碳水、纤维和添加糖确定性规则。
- [ ] 实现多约束求解与 `nutrition_constraints_infeasible` 结构化冲突原因。
- [ ] 建立版本化动作库、审核会话级 MET 映射和客户端禁止提交 MET 的完整门禁。
- [ ] 建立带来源、原始单位、食物状态、版本和审核时间的营养快照与食谱模板。
- [ ] 定义 `NutritionProvider` 接口；计算只使用审核缓存或 fixture，不访问实时付费 API。
- [ ] 覆盖营养边界、蛋白质分支、来源断裂、过敏原属性和无解场景测试。

### 代码与验证证据

- 当前 `feat/v1.0` 只有 `nutrition-policy-v1` 追溯元数据，不计算宏量营养素、食材重量、食谱或餐单。
...
- 正式生产常量仍需人工复核 `CN-DRI-2023` 正式表格。
- 营养数据商业授权、缓存许可和退出方案会阻塞生产数据接入，但不阻塞使用审核 fixture 开发确定性核心。
...
- 当前分支只产生阶段二 outbox 事件，尚无饮食重算消费端、库存或餐单领域模型。
