...
- 开发入口：用户已批准真实受控内测方向，书面设计为 `docs/superpowers/specs/2026-08-20-phase-7-cloud-integration-controlled-beta-design.md`；待用户复核设计后编写实施计划。阶段五、六的真实 CloudBase、供应商、双账号和设备事项继续作为阶段七/发布门禁，当前尚未勾选任何阶段七验收项。
...
- 2026-08-20 用户批准“同一阶段完成代码就绪与真实受控发布”的方向；书面设计固定 schema v8 可重试账户删除、白名单导出、生产审核数据集、助手摘要同事务一致性、隐私/AI 标识、发布预检、容量/恢复和双账号真机证据。设计复核、实施计划、代码和外部验收均尚未完成。

### 剩余工作

- 用户复核阶段七书面设计；随后编写并执行实施计划。
