...
| 7. 云端集成与内测发布 | 第 11–12 周 | 进行中 | 7A、7B 本地实现与质量门禁已完成；7C 发布手册、固定 CLI 预检和测试人员清单已就绪，但真实私有输入、云端发布及外部验收均未完成 |

## 当前阶段

- 当前阶段：阶段 7“云端集成与内测发布”（进行中）。
- 开发入口：用户已复核并批准 `docs/superpowers/specs/2026-08-20-phase-7-cloud-integration-controlled-beta-design.md`；实施拆为 `docs/superpowers/plans/2026-08-20-phase-7a-personal-data-rights.md`、`docs/superpowers/plans/2026-08-20-phase-7b-production-data-and-release-gates.md` 和 `docs/superpowers/plans/2026-08-20-phase-7c-controlled-beta-rollout.md`。7A、7B 已完成本地实现、自动化、发布门禁和交付说明；7C 已完成真实 CLI 命令面校正、发布手册和测试人员验收清单，下一步由发布/数据/合规负责人在受控本地环境提供私有输入后执行真实发布。真实 CloudBase、审核数据授权与导入、供应商、双账号、备份恢复和设备事项继续作为阶段七强制门禁，当前仍未勾选任何阶段七高层验收项。
...
- 7B 已进入 `feat/v1.0` 的实现提交：`d13521f`（严格审核数据合同、闭合图校验、SHA-256 与匿名证据）、`c366017`（CloudBase 精确数据集 Provider、60 秒缓存、2 秒超时与生产 fail-closed 装配）、`0bf48f7`（受控构建、发布预检和产物/敏感信息扫描）、`0377f5a`（3 MB/6 MB 容量基线、10×30 工作负载与 fail-fast `release:check`）、`a3821d8`（全仓 lint/测试 TypeScript 边界及并发构建回归门禁）。
- 7B 云运行时只读取 `planning_reviewed_datasets` 中与 `FITNESS_REVIEWED_DATASET_ID` 完全匹配的不可变文档；缺少配置、校验和/授权窗口/闭合引用图失败、超时或读失败均关闭为稳定 Provider 错误，不回退 fixture 或供应商运行时直连。候选验证、导入、激活和回滚操作契约见 `docs/cloudbase/phase-7-reviewed-dataset.md`。
- 2026-08-20 fresh 7B 本地门禁：`pnpm.cmd lint` 退出 0；`pnpm.cmd typecheck` 覆盖 13/14 个带脚本工作区、根级发布/端到端 TypeScript 和小程序严格检查；`pnpm.cmd test` 通过 104/104 个文件、970 项，另有 1 项 Windows 符号链接权限用例按设计跳过；`pnpm.cmd build`、三个 dry-run、`pnpm.cmd smoke:api`（2/2）、`pnpm.cmd smoke:assistant`（1/1）、`pnpm.cmd release:capacity`、`pnpm.cmd release:scan` 和 `git diff --check` 均退出 0。
- 容量证据使用精确 3,000,000 UTF-8 字节聚合，3,000,001 字节写入原子拒绝，导出小于 6 MB；10 个固定测试身份各 30 个真实 HTTP 结果共 300 个操作，规划成功率和 Provider 有界结果率均为 1，p95 为 741.65 ms，跨用户泄漏、部分事务、重复有效版本、清理丢失、Provider 超时、配额和预算超限计数均为 0。匿名证据只保存身份标签哈希和聚合指标。
- `release:dataset`、`release:preflight` 与完整 `release:check` 需要真实私有审核数据、授权证据、AppID/环境和服务端配置；本次未伪造输入运行通过，也未产生真实云发布结论。7B 只证明本地代码、发布工具和可部署产物就绪。
- 7C 准备提交 `7b5760b` 已按本机固定 `@cloudbase/cli@3.7.2` 实际帮助校正云函数与 NoSQL 备份/恢复命令，并把发布预检从不可解析的工作区 `pnpm exec tcb` 改为固定版本调用。修复后真实 `pnpm.cmd release:preflight` 能识别 CLI `3.7.2`，随后只因私有发布输入缺失而安全失败；未连接目标环境或执行任何外部写入。完整顺序见 `docs/cloudbase/phase-7-controlled-beta-release.md`。
- 7C 测试人员清单已覆盖双身份完整流程、健康成年人边界、餐单锁定/手改、受限助手、图片手动降级、严格导出与永久删除、固定错误、隐私/AI 标识、iOS/Android、网络恢复、容量/恢复及证据脱敏。清单存在只表示测试准备完成；发布负责人明确通知环境就绪前，测试人员不得开始真实验收。

### 剩余工作

- 7A、7B 无本地代码剩余项；7C 的发布手册、CLI 校正和测试人员清单已完成。发布/数据/合规负责人仍需在受控本地环境提供真实私有输入，不得粘贴到仓库、缺陷单或聊天。
- 尚缺真实审核数据文件、数据集 ID、来源授权与正式 `CN-DRI-2023` 复核记录；真实小程序 AppID、专用 CloudBase 环境 ID、服务端配置名、公开运营主体/隐私联系方式/隐私说明版本和发布时间；CloudBase 容量输入、备份恢复检查时间、两个隔离账号、十个容量账号、iOS/Android 设备以及供应商/合规审批。
- 上述输入到位后仍须依次完成数据集离线校验与导入/激活、真实 `release:preflight`/`release:check`、三函数部署、索引/规则/IAM/定时器、双账号/供应商/数据权利、日志/容量/恢复/回滚和真机验收，才能交付测试人员正式签署。

### 外部阻塞

- 数据负责人：提供真实审核数据文件、不可变数据集 ID、来源授权/缓存/展示/退出证据和正式 `CN-DRI-2023` 复核记录。
- 发布负责人：在专用受控内测环境配置真实 AppID、CloudBase 环境 ID、全部服务端配置名、公开运营/隐私元数据、容量输入与备份恢复检查时间，并准备两个隔离身份、十个容量身份和 iOS/Android 设备。
- 合规/供应商负责人：关闭模型备案或登记、AI 内容标识、微信隐私材料、营养/视觉/模型供应商授权、费用和数据处理审批。
- 在上述负责人确认目标环境、权限和输入前，7C 不得运行数据导入、部署、规则/索引/定时器、备份恢复或账户删除等外部写操作。
